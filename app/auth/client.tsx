...
export default function Client({ redirectTo, flashStatus, flashMessage }: ClientProps) {
  ...
  const content = useMemo(() => {
    if (mode === "signup") {
      return {
        id: "signup",
        title: "Create your LeadBase account",
        description: "Start organizing contacts and deals",
        mainBtnLabel: "Create Account",
        helper: "Already have an account? Sign in.",
        helperLink: "/auth#signin",
    ...
    return {
      id: "signin",
      title: "Sign in to LeadBase",
      description: "Access your CRM dashboard",
      mainBtnLabel: "Sign In",
      helper: "Don't have an account? Sign up for free.",
      helperLink: "/auth#signup",
          ...
          <div className="relative z-10 flex h-full flex-col justify-between">
            <div className="space-y-4">
              ...
              <h1 className="max-w-sm text-4xl font-semibold leading-tight tracking-tight">
                The simple CRM to organize your contacts and deals.
              </h1>
              <p className="max-w-md text-sm text-muted-foreground">
                Fast onboarding, secure authentication, and a distraction-free dashboard to help you grow smarter.
              </p>
            </div>
            ...
            <CardContent className="space-y-6">
              ...
              {mode === "signin" ? (
                <form className="space-y-4" action={signInAction}>
                  ...
                  <Button type="submit" className="w-full" disabled={isPending}>
                    {isPending ? "Signing in..." : content.mainBtnLabel}
                  </Button>
                </form>
              ) : (
                <form className="space-y-4" action={signUpAction}>
                  ...
                  <Button type="submit" className="w-full" disabled={isPending}>
                    {isPending ? "Creating account..." : content.mainBtnLabel}
                  </Button>
                </form>
              )}
              ...
              {activeState.status === "success" && activeState.message ? (
                <p className="text-sm font-medium text-emerald-700 dark:text-emerald-400" role="status">
                  {activeState.message === "Check your inbox for a confirmation email."
                    ? "Check your inbox for a LeadBase confirmation email."
                    : activeState.message}
                </p>
              ) : null}
              ...
              <div className="text-center pt-4">
                <span className="text-sm text-muted-foreground">{content.helper}{" "}</span>
                <Link
                  href={content.helperLink}
                  className="text-sm font-semibold text-primary hover:underline"
                >
                  {content.helperLinkText}
                </Link>
              </div>
            </CardContent>
          </Card>
